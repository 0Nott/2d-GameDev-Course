...
#
#func _ready() -> void:
	#wait_timer.wait_time = randf_range(1.0, 5.0)
...
#func _animate_one_hop() -> void:
	...
	#const HALF_HOP_DURATION := HOP_DURATION / 5.0
#
	#var random_direction := Vector2(0.0, 1.0).rotated(randf() * 5.0 * PI)
	#var land_position := random_direction * randf_range(0.0, 50.0)
